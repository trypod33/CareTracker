...
class MedicationsFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
            ...
            vm.filterOptions.collect { options ->
                ...
                options.forEach { label ->
                    val chip = Chip(requireContext()).apply {
                        text = label
                        isCheckable = true
                        isChecked = label == vm.filter.value
        ...
        viewLifecycleOwner.lifecycleScope.launch {
            vm.filter.collect { active ->
                binding.chipGroupFilter.children.filterIsInstance<Chip>().forEach {
                    it.isChecked = it.text == active
                }
            }
        }
    ...
    private fun showAddEditDialog(existing: MedicationEntity?) {
        ...
        existing?.let { med ->
            ...
            mapOf("#4f9cf9" to db.rbBlue,"#9b59b6" to db.rbPurple,"#2ecc71" to db.rbGreen,
                  "#e67e22" to db.rbOrange,"#e74c3c" to db.rbRed,"#e91e8c" to db.rbPink,
                  "#ff9800" to db.rbAmber,"#00bcd4" to db.rbCyan)[med.color]?.isChecked = true
        }
        ...
        AlertDialog.Builder(requireContext())
            ...
            .setPositiveButton(if (existing != null) "Save" else "Add") { _, _ ->
                val color = when {
                    db.rbBlue.isChecked   -> "#4f9cf9"; db.rbPurple.isChecked -> "#9b59b6"
                    db.rbGreen.isChecked  -> "#2ecc71"; db.rbOrange.isChecked -> "#e67e22"
                    db.rbRed.isChecked    -> "#e74c3c"; db.rbPink.isChecked   -> "#e91e8c"
                    db.rbAmber.isChecked  -> "#ff9800"; db.rbCyan.isChecked   -> "#00bcd4"
                    else -> "#4f9cf9"
                }
