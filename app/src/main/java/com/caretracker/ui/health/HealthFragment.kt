...
class HealthFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, s: Bundle?) {
        ...
        b.tvHealthDate.text = LocalDate.now().format(DateTimeFormatter.ofLocalizedDate(FormatStyle.FULL))
    ...
    private fun updateCards(e: HealthEntryEntity?) {
        ...
        b.tvBloodPressure.text = if (e?.bloodPressureSystolic != null && e.bloodPressureDiastolic != null) "${e.bloodPressureSystolic}/${e.bloodPressureDiastolic}" else "--/--"
    ...
    private fun showDialog(existing: HealthEntryEntity?) {
        ...
        d.etDate.setOnClickListener {
            val p = date.split("-"); val cal = Calendar.getInstance()
            if (p.size == 3) cal.set(p[0].toInt(), p[1].toInt()-1, p[2].toInt())
            DatePickerDialog(requireContext(), { _, y, m, day ->
                date = "%04d-%02d-%02d".format(y, m+1, day); d.etDate.setText(fmt(date))
            }, cal.get(Calendar.YEAR), cal.get(Calendar.MONTH), cal.get(Calendar.DAY_OF_MONTH)).show()
        }
        val units = listOf("lbs","kg")
        d.spinnerWeightUnit.adapter = ArrayAdapter(requireContext(), android.R.layout.simple_spinner_item, units).also { it.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item) }
        ...
        existing?.let { e ->
            e.weight?.let { d.etWeight.setText("%.1f".format(it)) }
            e.heartRate?.let { d.etHeartRate.setText(it.toString()) }
            e.bloodPressureSystolic?.let { d.etSystolic.setText(it.toString()) }
            e.bloodPressureDiastolic?.let { d.etDiastolic.setText(it.toString()) }
            e.bloodSugar?.let { d.etBloodSugar.setText("%.0f".format(it)) }
            e.sleepHours?.let { d.etSleep.setText("%.1f".format(it)) }
            e.sleepQuality?.let { d.seekSleepQuality.progress=(it-1).coerceIn(0,9); d.tvSleepQuality.text=it.toString() }
            e.mood?.let { d.seekMood.progress=(it-1).coerceIn(0,9); d.tvMood.text=it.toString() }
            e.energy?.let { d.seekEnergy.progress=(it-1).coerceIn(0,9); d.tvEnergy.text=it.toString() }
            e.steps?.let { d.etSteps.setText(it.toString()) }
            e.exerciseMinutes?.let { d.etExercise.setText(it.toString()) }
            e.waterOz?.let { d.etWater.setText("%.1f".format(it)) }
            e.calories?.let { d.etCalories.setText(it.toString()) }
            e.notes?.let { d.etNotes.setText(it) }
        }
        val dlg = AlertDialog.Builder(requireContext(), R.style.FullWidthDialog).setView(d.root).create()
        dlg.window?.setBackgroundDrawableResource(android.R.color.transparent)
        d.btnCancel.setOnClickListener { dlg.dismiss() }
        d.btnSave.setOnClickListener {
            vm.saveEntry(HealthEntryEntity(id=existing?.id?:0L, userId=app.currentUserId, entryDate=date,
                weight=d.etWeight.text?.toString()?.toFloatOrNull(), weightUnit=units[d.spinnerWeightUnit.selectedItemPosition],
                ...
                sleepQuality=d.seekSleepQuality.progress+1, mood=d.seekMood.progress+1, energy=d.seekEnergy.progress+1,
                steps=d.etSteps.text?.toString()?.toIntOrNull(), exerciseMinutes=d.etExercise.text?.toString()?.toIntOrNull(),
                waterOz=d.etWater.text?.toString()?.toFloatOrNull(), calories=d.etCalories.text?.toString()?.toIntOrNull(),
    ...
    private fun confirmDelete(e: HealthEntryEntity) {
        AlertDialog.Builder(requireContext()).setTitle("Delete Entry").setMessage("Delete entry for ${fmt(e.entryDate)}?")
            .setPositiveButton("Delete") { _, _ -> vm.deleteEntry(e) }.setNegativeButton("Cancel", null).show()
    }
    private fun fmt(iso: String) = runCatching { LocalDate.parse(iso).format(DateTimeFormatter.ofPattern("MM/dd/yyyy")) }.getOrDefault(iso)
